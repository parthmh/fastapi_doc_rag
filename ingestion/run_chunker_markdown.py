...
def load_page_tree(debug_page_dir: Path) -> PageTree:
    ...
    flat_nodes = [FlatNode(**node) for node in headings_raw.values()]
    admonition_nodes = [AdmonitionNode(**node) for node in admonitions_raw.values()]

    nodes_by_id = {node.node_id: node for node in flat_nodes}
    admonition_nodes_by_id = {node.node_id: node for node in admonition_nodes}
...
def main() -> None:
    if not DEBUG_PARSE_ROOT.exists():
        raise FileNotFoundError(
            f"Parse debug root not found: {DEBUG_PARSE_ROOT}. "
    ...
    for debug_page_dir in iter_debug_page_dirs(DEBUG_PARSE_ROOT):
        ...
        output_path = write_chunks_for_page(page_id_slug, chunks)
